-- Merci a Fellow pour cette partie du script :) ! , c'est une version modifié mais le gros du travail a été fait par lui
-- et le mérite lui en revient pleinement.
...
AddEventHandler('fellow:MenuFouille', function()
    ...
    local target_id = GetPlayerServerId(target)
    local searchPlayerPed = GetPlayerPed(target)
    if IsEntityPlayingAnim(searchPlayerPed, 'random@mugging3', 'handsup_standing_base', 3) or  closestDistance <= 3.0 and closestPlayer ~= 1 then
    MenuFouille()
else
    ESX.ShowNotification('~r~ERREUR~s~: Aucun citoyen proche ou la personne ne lève pas les bras')
end
end)
